...
EX  int animation_load(int id, const char *name)
    {
        auto a = animation::get(id);
        if (!a->anim->load(name))
            return -1;
        else
            return a->anim->get_duration();
    }
...
EX  void render_shadows_size(float size) { scene::instance().set_shadows_size(size); }
